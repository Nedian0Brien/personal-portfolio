...
const EDIT_BACKUP_DIR = path.resolve(__dirname, "../.runtime/edit-backups");
...
function writeTextEdit({ sourceText, replacementText }) {
  ...
  fs.copyFileSync(INDEX_HTML_PATH, path.join(EDIT_BACKUP_DIR, `index-${Date.now()}.html`));
  fs.writeFileSync(INDEX_HTML_PATH, source.replace(sourceText, preserveOuterWhitespace(sourceText, replacementText)));
