...
**No AI Slop 규칙.** neon blue, 장식적 AI glow, 장식용 glass panel을 추가하지 않는다. 기존 gradient는 project preview world와 논문 상세 hero title의 `약물–부작용` 강조처럼 명확한 semantic emphasis에 연결될 때만 허용된다.
...
- **Don't** colored side-stripe border와 decorative glassmorphism을 사용하지 않는다. gradient text는 논문 상세 hero title의 좁은 semantic emphasis 예외를 제외하고 사용하지 않는다.
